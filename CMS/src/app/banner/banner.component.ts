...
export class BannerComponent implements OnInit {
  ...
  onGetToList() {
    this.isShowLoading = true;
    this.BannerService.GetByParentIDToListAsync(this.parentID).subscribe(
      res => {
        this.BannerService.list = res as Banner[];
        this.dataSource = new MatTableDataSource(this.BannerService.list.sort((a, b) => (a.SortOrder > b.SortOrder ? 1 : -1)));
  ...
  onDelete(element: Banner) {
    if (confirm(element.Name + ': ' + environment.DeleteConfirm)) {
      this.BannerService.RemoveAsync(element.ID).subscribe(
        res => {
          this.onSearch();
