...
export class NewsletterComponent implements OnInit {
  ...
  onGetToList() {
    this.isShowLoading = true;
    this.NewsletterService.GetByParentIDToListAsync(this.parentID).subscribe(
      res => {
        this.NewsletterService.list = res as Newsletter[];
        this.dataSource = new MatTableDataSource(this.NewsletterService.list.sort((a, b) => (a.SortOrder > b.SortOrder ? 1 : -1)));
  ...
  onDelete(element: Newsletter) {
    if (confirm(element.Name + ': ' + environment.DeleteConfirm)) {
      this.NewsletterService.RemoveAsync(element.ID).subscribe(
        res => {
          this.onSearch();
